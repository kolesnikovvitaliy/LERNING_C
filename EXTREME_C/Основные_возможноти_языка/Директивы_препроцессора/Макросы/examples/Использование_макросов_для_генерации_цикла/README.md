...
Диаграмма ниже наглядно иллюстрирует, как разворачиваются синтаксические макросы во время фазы препроцессинга и как именно выполняется функция `execute_linear_range_counter` (`main`):
...
* **Имя функции**: `main` (Логическое имя: `execute_linear_range_counter`)
...
<details>
<summary>📂 Посмотреть main.c со спецификацией Doxygen</summary>
...
После того как препроцессор обработает заголовочные файлы и подставит значения всех макросов (эквивалентно вызову команды `gcc -E main.c`), функция `main` принимает классический вид стандартного языка Си:
